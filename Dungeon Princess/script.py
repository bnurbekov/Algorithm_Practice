...
class Solution:
    ...
    def calculateMinimumHP(self, A):
        D = [[None]*(len(A)+1) for i in xrange(len(A)+1)]
        parent = [[None]*(len(A)) for i in xrange(len(A))] 

        for i in xrange(len(A)+1):
            for j in xrange(len(A)+1):
                if i == 1 and j == 1:
                    D[i][j] = max(1-A[i-1][j-1], 1)
                elif i == 0 or j == 0:
                    D[i][j] = sys.maxint
                else:
                    if D[i-1][j] >= D[i][j-1]:
                        mini = D[i][j-1]
                        parent[i-1][j-1] = (i-1, j-2)
                    else:
                        mini = D[i-1][j]
                        parent[i-1][j-1] = (i-2, j-1)

                    D[i][j] = max(-A[i-1][j-1] + mini, 1)

        temp = (i-1, j-1)
        print parent
        print parent[i-1][j-1]
        path = []
        while temp != None:
        	path.append(temp)
        ...
        initHealth = 1009
        for pos in path:
        	initHealth += A[pos[0]][pos[1]]
        	print "H:", initHealth, "PS:", pos, "PR:", parent[pos[0]][pos[1]], A[pos[0]][pos[1]], D[pos[0]+1][pos[1]+1]
