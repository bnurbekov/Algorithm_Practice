...
class Solution:
    ...
    def jump(self, A):
        ...
        for i in xrange(len(A)-2, -1, -1):
            minJumps = sys.maxint
            for j in xrange(min(len(A)-1, i+A[i]), i, -1):
                minJumps = min(minJumps, 1+cache[j])

            if minJumps != sys.maxint:
            	print "Min Jumps", minJumps
                cache[i] = minJumps
                
        return cache[0]

    def jumpGreedy(self, A):
        # cache = [-1]*len(A)
        # cache[len(A)-1] = 0
